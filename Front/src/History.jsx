				...
				{loading ? (
					...
					<ul className="space-y-2">
						{filtered.map((item) => (
							<li
								key={item._id}
								className="rounded-lg shadow-sm bg-white hover:bg-gray-100 transition"
							>
								<Link
									to={`/ride-details/${item._id}`}
									className="flex items-center justify-between p-4 w-full block"
								>
									<div className="flex items-center">
										...
										<div>
											...
											<p className="text-sm text-gray-500">
												From{' '}
												{item.startLocation?.startLocationName || 'Unknown'} to{' '}
												{item.endLocation?.endLocationName || 'Unknown'}
											</p>
										</div>
									</div>
